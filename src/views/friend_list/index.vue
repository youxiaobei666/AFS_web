<template>
  <div>
    <el-button
      v-if="showType !== 'list'"
      @click="(evt) => (showType = 'list')"
      class="back-btn"
      ><el-icon><ArrowLeftBold /></el-icon> 返回好友列表</el-button
    >
    <!--状态一：好友列表-->
    <GoodFriendList
      v-if="showType === 'list'"
      :friends="store.getters.getFriendsList || []"
      @contact-friend="goToChat"
...
const deleteFriend = (fid) => {
  editUserInfo({
    ...
    friends: store.getters.userInfo.friends
      .split(',')
      .filter((i) => Number.parseInt(i) !== Number.parseInt(fid))
      .join(','),
  })
}
</script>
